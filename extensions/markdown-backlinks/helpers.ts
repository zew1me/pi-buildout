...
export function formatBacklinkTable(backlinks: MarkdownBacklink[]): string {
	...
	const rows = backlinks
		...
		.join("\n");
	return `\n\n## Markdown file pointers\n\nThe following file pointers were found in Markdown files. Treat each as a reasonable candidate for a \`read\` tool call when relevant. The pointer remains listed until the target is read successfully or is found to be missing; missing targets are removed.\n\n${rows}`;
}
