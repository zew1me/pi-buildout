import { type Api, complete, type Model, type Tool, validateToolArguments } from "@earendil-works/pi-ai/compat";
...
const SECONDARY_IDS: Record<ModelVendor, readonly (readonly [string, string])[]> = {
	openai: [
...
export function selectClassifierModels(models: readonly Model<Api>[]): {
	...
	const secondary = primary ? findConfiguredModel(models, SECONDARY_IDS[primary.vendor]) : undefined;
	return {
		...(primary ? { primary } : {}),
		...(secondary ? { secondary } : {}),
	};
...
function transportFor(ctx: ExtensionContext, selected: ClassifierModel | undefined): ClassifierTransport {
	return async (request) => {
		...
		const validatedArguments = validateToolArguments(CLASSIFIER_TOOL, toolCall);
		return {
