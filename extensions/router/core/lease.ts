...
import { type EffortLevel, findPromptProfile } from "./profiles.ts";
...
export function installLease(state: LeaseState, lease: TaskLease): LeaseState {
	const { pendingHardBoundary: _pendingHardBoundary, ...rest } = state;
...
export function changeEffortWithinLease(
	...
	if (!profile || profile.id !== lease.promptProfileId) {
...
export function createChildLease(parent: TaskLease, child: TaskLease): TaskLease {
	return { ...child, parentTaskId: parent.taskId };
}
